...
from NCP_MFVI.common_Darcy import EquSolver, ModelDarcyFlow
from NCP_MFVI.common_Darcy import EquSolver_hybird
...
equ_nx = 50
domain_equ = Domain2D(nx=equ_nx, ny=equ_nx, mesh_type='P', mesh_order=1)
...
d_est = model.S @ equ_solver.forward_solver(m_newton_cg.vector()[:])
...
## save the m_Map
np.save(DATA_DIR + "m_Map.npy", m_newton_cg.vector()[:])

## save p0 = H_fwd(m_Map)
p0 = equ_solver.forward_solver(m_newton_cg.vector()[:])
np.save(DATA_DIR + "p0.npy", p0)




# save d_clean and d of the linear part

p0 = np.load(DATA_DIR + "p0.npy")

d_linear = d - equ_solver.S @ p0
d_linear_clean = d_clean - equ_solver.S @ p0
plt.figure()
plt.plot(d_linear)
np.save(DATA_DIR + 'measurement_2D_linear.npy', d_linear)
np.save(DATA_DIR + 'measurement_2D_linear_clean.npy', d_linear_clean)
